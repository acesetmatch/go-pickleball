...
func InitDB() error {
	...
	// Tables are created via migrations - run `make migrate` first
...
func GetPaddleByID(paddleId string) (*Paddle, error) {
	...
	row := DB.QueryRow(`
		SELECT 
			p.paddle_id, p.brand, p.model, p.image_url, p.buy_url, p.price,
	...
	err := row.Scan(
		&paddle.ID, &paddle.Metadata.Brand, &paddle.Metadata.Model, &paddle.ImageURL, &paddle.BuyURL, &paddle.Price,
...
func SavePaddle(paddle *Paddle) (int, error) {
	...
	err = tx.QueryRow(`
		INSERT INTO paddles (
			paddle_id, brand, model, image_url, buy_url, price
		) VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id
	`,
		paddle.ID, paddle.Metadata.Brand, paddle.Metadata.Model, paddle.ImageURL, paddle.BuyURL, paddle.Price,
