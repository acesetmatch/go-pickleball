...
def main():
    ...
    from matts_scraper import MattsPickleballScraper
    ...
    # Test Matt's scraper
    matts_scraper = MattsPickleballScraper()
    matts_paddles = matts_scraper.scrape_all()
    log_success(logging.getLogger(), f"Matt's scraper found {len(matts_paddles)} paddles")
    
    # Save Matt's paddles to JSON
    if matts_paddles:
        matts_data = [asdict(paddle) for paddle in matts_paddles]
        with open('scraped_paddles_matts.json', 'w') as f:
            json.dump(matts_data, f, indent=2)
        log_success(logging.getLogger(), f"Saved {len(matts_paddles)} Matt's paddles to scraped_paddles_matts.json")
    
    # Test Firecrawl scraper for Matt's paddle finder
    try:
        firecrawl_service = FirecrawlScrapingService()
        firecrawl_paddles = firecrawl_service.scrape_and_save()
        log_success(logging.getLogger(), f"Firecrawl scraper found {len(firecrawl_paddles)} paddles from Matt's paddle finder")
    except ValueError as e:
        logging.getLogger().warning(f"Firecrawl scraper skipped: {e}")
    except Exception as e:
        logging.getLogger().error(f"Firecrawl scraper failed: {e}")
